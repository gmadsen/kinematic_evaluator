...
kineval.applyControls = function robot_apply_controls() {
    ...
    var lolim = robot.joints[x].limit["lower"];
    var uplim = robot.joints[x].limit["upper"];
    robot.joints[x].angle = Math.min(Math.max(robot.joints[x].angle,lolim),uplim);
    }
    if (robot.joints[x].type == 'fixed') robot.joints[x].angle = 0;
